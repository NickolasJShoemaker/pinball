...
public:
  ...
  void onTick(){
	...
	if (m_aArrow[3]){
	  m_aArrow[3] = false;
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[4], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[3], 0, this->getParent(), NULL );
	  m_iRampTick = 0;
	} else if (m_aArrow[2]){
	  m_aArrow[2] = false;
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[3], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[2], 0, this->getParent(), NULL );
	  m_iRampTick = 0;
	}
      }      
    }
	...
	if (m_aArrow[6]){
	  m_aArrow[6] = false;
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[7], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[6], 0, this->getParent(), NULL );
	  m_iRampTick = 0;
	} else if (m_aArrow[5]){
	  m_aArrow[5] = false;
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[6], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[5], 0, this->getParent(), NULL );
	  m_iRampTick = 0;
	}
      }      
    }
  ...
  void onSignal(int em_signal, Group* em_group){
    ...
    // ball dead
      ...
      if (table->active() == 1){
	// multiball is dead
	SignalSender::getInstance()->addSignal( m_sigMultiballOff, 0, this->getParent(), NULL );
	m_bMultiBallOn = false;
      }
      if (table->active() == 0){
	// no active ball
	SignalSender::getInstance()->addSignal( m_sigAllBallsOff, 0, this->getParent(), NULL );
	if (m_bExtraBall){
	  m_bExtraBall = false;
	...
	if (m_iBumpCounter == 10){
	  ...
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[0], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[1], 0, this->getParent(), NULL );
	...
	if (m_iBumpCounter == 10){
	  ...
	  SignalSender::getInstance()->addSignal( m_sigSideTargetBlink[0], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigSideTargetBlink[1], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigSideTargetBlink[2], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigSideTargetBlink[3], 0, this->getParent(), NULL );
	...
	if (m_iBumpCounter == 10){
	  ...
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[2], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[3], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[4], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[5], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[6], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[7], 0, this->getParent(), NULL );
	  //cerr << "right ramp arrows blink" << endl;
	  m_iMissionTick = MISSION_TEXT_TIMEOUT;
	}
      }
    ...
    // cave
    else if(em_signal == m_sigCave){
      if (m_iMission == MISSION_FIRST_CAVE){
	SignalSender::getInstance()->addSignal( m_sigBigTuxOn[0], 0, this->getParent(), NULL );
	...
	SignalSender::getInstance()->addSignal( m_sigBigTuxOn[1], 0, this->getParent(), NULL );
	...
	SignalSender::getInstance()->addSignal( m_sigBigTuxOn[2], 0, this->getParent(), NULL );
	...
	SignalSender::getInstance()->addSignal( m_sigBigTuxOn[3], 0, this->getParent(), NULL );
	...
	SignalSender::getInstance()->addSignal( m_sigMissionAll, 0, this->getParent(), NULL );
	//SignalSender::getInstance()->addSignal( m_sigStopBalls, 0, this->getParent(), NULL );
	//SignalSender::getInstance()->addSignal( m_sigResumeBalls, 1000, this->getParent(), NULL );
    ...
    // LINUX
    else if(em_signal == m_sigLinux[0]){
      if (!m_aLinux[0]){
	SignalSender::getInstance()->addSignal(m_sigLinuxOn[0], 0, this->getParent(), NULL);
	m_aLinux[0] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal == m_sigLinux[1]){
      if (!m_aLinux[1]){
	SignalSender::getInstance()->addSignal(m_sigLinuxOn[1], 0, this->getParent(), NULL);
	m_aLinux[1] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal == m_sigLinux[2]){
      if (!m_aLinux[2]){
	SignalSender::getInstance()->addSignal(m_sigLinuxOn[2], 0, this->getParent(), NULL);
	m_aLinux[2] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal == m_sigLinux[3]){
      if (!m_aLinux[3]){
	SignalSender::getInstance()->addSignal(m_sigLinuxOn[3], 0, this->getParent(), NULL);
	m_aLinux[3] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal == m_sigLinux[4]){
      if (!m_aLinux[4]){
	SignalSender::getInstance()->addSignal(m_sigLinuxOn[4], 0, this->getParent(), NULL);
	m_aLinux[4] = true;
      }
      score->addScore(50);
    } 
    // TUX
    else if(em_signal == m_sigTux[0]){
      if (!m_aTux[0]){
	SignalSender::getInstance()->addSignal(m_sigTuxOn[0], 0, this->getParent(), NULL);
	m_aTux[0] = true;
      }
      score->addScore(500);
    } 
    else if(em_signal == m_sigTux[1]){
      if (!m_aTux[1]){
	SignalSender::getInstance()->addSignal(m_sigTuxOn[1], 0, this->getParent(), NULL);
	m_aTux[1] = true;
      }
      score->addScore(500);
    } 
    else if(em_signal == m_sigTux[2]){
      if (!m_aTux[2]){
	SignalSender::getInstance()->addSignal(m_sigTuxOn[2], 0, this->getParent(), NULL);
	m_aTux[2] = true;
      }
      score->addScore(500);
    } 
    // BOOT
    else if(em_signal ==  m_sigBoot[0] ){
      if (m_aBoot[0]){
	SignalSender::getInstance()->addSignal(m_sigBootOff[0], 0, this->getParent(), NULL);
	m_aBoot[0] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigBootOn[0], 0, this->getParent(), NULL);
	m_aBoot[0] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal ==  m_sigBoot[1] ){
      if (m_aBoot[1]){
	SignalSender::getInstance()->addSignal(m_sigBootOff[1], 0, this->getParent(), NULL);
	m_aBoot[1] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigBootOn[1], 0, this->getParent(), NULL);
	m_aBoot[1] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal ==  m_sigBoot[2] ){
      if (m_aBoot[2]){
	SignalSender::getInstance()->addSignal(m_sigBootOff[2], 0, this->getParent(), NULL);
	m_aBoot[2] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigBootOn[2], 0, this->getParent(), NULL);
	m_aBoot[2] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal ==  m_sigBoot[3] ){
      if (m_aBoot[3]){
	SignalSender::getInstance()->addSignal(m_sigBootOff[3], 0, this->getParent(), NULL);
	m_aBoot[3] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigBootOn[3], 0, this->getParent(), NULL);
	m_aBoot[3] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal ==  PBL_SIG_LEFTARM_ON ){
      // BOOT
      bool tmp = m_aBoot[0];
      for (int a=0; a<3; ++a){
	if (m_aBoot[a] != m_aBoot[a+1]){
	  m_aBoot[a] = m_aBoot[a+1];
	  if (m_aBoot[a]) SignalSender::getInstance()->addSignal(m_sigBootOn[a], 0, this->getParent(), NULL);
	  else SignalSender::getInstance()->addSignal(m_sigBootOff[a], 0, this->getParent(), NULL);
	} 
      } 
      if (m_aBoot[3] != tmp){
	m_aBoot[3] = tmp;
	if (m_aBoot[3]) SignalSender::getInstance()->addSignal(m_sigBootOn[3], 0, this->getParent(), NULL);
	else SignalSender::getInstance()->addSignal(m_sigBootOff[3], 0, this->getParent(), NULL);
      }
      // LINUX
      tmp = m_aLinux[0];
	...
	if (m_aLinux[aa] != m_aLinux[aa+1]){
	  ...
	  if (m_aLinux[aa]) SignalSender::getInstance()->addSignal(m_sigLinuxOn[aa], 0, this->getParent(), NULL);
	  else SignalSender::getInstance()->addSignal(m_sigLinuxOff[aa], 0, this->getParent(), NULL);
	}
      } 
      if (m_aLinux[4] != tmp){
	m_aLinux[4] = tmp;
	if (m_aLinux[4]) SignalSender::getInstance()->addSignal(m_sigLinuxOn[4], 0, this->getParent(), NULL);
	else SignalSender::getInstance()->addSignal(m_sigLinuxOff[4], 0, this->getParent(), NULL);
      }
    } 
    else if(em_signal ==  PBL_SIG_RIGHTARM_ON ){
      // BOOT
      bool tmp = m_aBoot[3];
      { for (int a=3; a>0; --a){
	if (m_aBoot[a] != m_aBoot[a-1]){
      ...
      if (m_aBoot[0] != tmp){
	m_aBoot[0] = tmp;
	if (m_aBoot[0]) SignalSender::getInstance()->addSignal(m_sigBootOn[0], 0, this->getParent(), NULL);
	else SignalSender::getInstance()->addSignal(m_sigBootOff[0], 0, this->getParent(), NULL);
      }
      // LINUX
      tmp = m_aLinux[4];
      { for (int a=4; a>0; --a){
	if (m_aLinux[a] != m_aLinux[a-1]){
	  ...
	  if (m_aLinux[a]) SignalSender::getInstance()->addSignal(m_sigLinuxOn[a], 0, this->getParent(), NULL);
	  else SignalSender::getInstance()->addSignal(m_sigLinuxOff[a], 0, this->getParent(), NULL);
	}
      } }
      if (m_aLinux[0] != tmp){
	m_aLinux[0] = tmp;
	if (m_aLinux[0]) SignalSender::getInstance()->addSignal(m_sigLinuxOn[0], 0, this->getParent(), NULL);
	else SignalSender::getInstance()->addSignal(m_sigLinuxOff[0], 0, this->getParent(), NULL);
      }
    } 
    else if(em_signal == m_sigRightLoop){
      if (m_bExtraBallWaiting){
	SignalSender::getInstance()->addSignal( m_sigExtraBall, 0, this->getParent(), NULL );
	m_bExtraBall = true;
	...
	m_iRampTick = 0;
	if (m_aArrow[6]){
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[5], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[6], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[7], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigBigTuxBlink[3], 0, this->getParent(), NULL );
	  m_iMission = MISSION_FOURTH_CAVE;
	  SignalSender::getInstance()->addSignal( m_sigMissionCave, 0, this->getParent(), NULL );
	  m_iMissionTick = MISSION_TEXT_TIMEOUT;
	  m_aArrow[5] = m_aArrow[6] = m_aArrow[7] = false;
	} else if (m_aArrow[5]){
	  m_aArrow[6] = true;
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[7], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowOn[6], 0, this->getParent(), NULL );
	}else{
	  m_aArrow[5] = true;
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[6], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowOn[5], 0, this->getParent(), NULL );
	}
      }
      if (m_iMission == MISSION_FIFTH_ALL){
	SignalSender::getInstance()->addSignal( m_sigArrowOn[5], 0, this->getParent(), NULL );
	SignalSender::getInstance()->addSignal( m_sigArrowOn[6], 0, this->getParent(), NULL );
	SignalSender::getInstance()->addSignal( m_sigArrowOn[7], 0, this->getParent(), NULL );
	m_aArrow[5] = m_aArrow[6] = m_aArrow[7] = true;
      }
    } 
    else if(em_signal == m_sigLeftLoop){
      if (m_bMultiBallOn){
	SignalSender::getInstance()->addSignal( m_sigJackpot, 0, this->getParent(), NULL );
	...
	m_iRampTick = 0;
	if (m_aArrow[3]){
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[2], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[3], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[4], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigBigTuxBlink[2], 0, this->getParent(), NULL );
	  ...
	  m_aArrow[3] = true;
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[4], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowOn[3], 0, this->getParent(), NULL );
	}else{
	  m_aArrow[2] = true;
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[3], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowOn[2], 0, this->getParent(), NULL );
	}
      }
      if (m_iMission == MISSION_FIFTH_ALL){
	SignalSender::getInstance()->addSignal( m_sigArrowOn[2], 0, this->getParent(), NULL );
	SignalSender::getInstance()->addSignal( m_sigArrowOn[3], 0, this->getParent(), NULL );
	SignalSender::getInstance()->addSignal( m_sigArrowOn[4], 0, this->getParent(), NULL );
	m_aArrow[2] = m_aArrow[3] = m_aArrow[4] = true;
      }
    }
    // Side Targets
    else if(em_signal == m_sigSideTarget[0]){
      if (m_iMission == MISSION_SIDETARGETS || m_iMission == MISSION_FIFTH_ALL){
	m_aSideTarget[0] = true;
      }
      score->addScore(350*m_iMultiplier);
    }
    else if(em_signal == m_sigSideTarget[1]){
      if (m_iMission == MISSION_SIDETARGETS || m_iMission == MISSION_FIFTH_ALL){
	m_aSideTarget[1] = true;
      }
      score->addScore(350*m_iMultiplier);
    }
    else if(em_signal == m_sigSideTarget[2]){
      if (m_iMission == MISSION_SIDETARGETS || m_iMission == MISSION_FIFTH_ALL){
	m_aSideTarget[2] = true;
      }
      score->addScore(350*m_iMultiplier);
    }
    else if(em_signal == m_sigSideTarget[3]){
      if (m_iMission == MISSION_SIDETARGETS || m_iMission == MISSION_FIFTH_ALL){
    ...
    // Arrows
    else if(em_signal == m_sigArrow[0]){
      if (m_iMission == MISSION_ARROWS || m_iMission == MISSION_FIFTH_ALL){
	SignalSender::getInstance()->addSignal( m_sigArrowOn[0], 0, this->getParent(), NULL );
	m_aArrow[0] = true;
      }
      score->addScore(750);
    }
    else if(em_signal == m_sigArrow[1]){
      if (m_iMission == MISSION_ARROWS || m_iMission == MISSION_FIFTH_ALL){
	SignalSender::getInstance()->addSignal( m_sigArrowOn[1], 0, this->getParent(), NULL );
	m_aArrow[1] = true;
      }
      score->addScore(750);
    }
    ...
    if (m_aArrow[0] && m_aArrow[1] && m_iMission == MISSION_ARROWS){
      SignalSender::getInstance()->addSignal( m_sigArrowOff[0], 0, this->getParent(), NULL );
      SignalSender::getInstance()->addSignal( m_sigArrowOff[1], 0, this->getParent(), NULL );
      SignalSender::getInstance()->addSignal( m_sigBigTuxBlink[0], 0, this->getParent(), NULL );
	...
	m_iMission == MISSION_SIDETARGETS){
      SignalSender::getInstance()->addSignal( m_sigSideTargetOff[0], 0, this->getParent(), NULL );
      SignalSender::getInstance()->addSignal( m_sigSideTargetOff[1], 0, this->getParent(), NULL );
      SignalSender::getInstance()->addSignal( m_sigSideTargetOff[2], 0, this->getParent(), NULL );
      SignalSender::getInstance()->addSignal( m_sigSideTargetOff[3], 0, this->getParent(), NULL );
      SignalSender::getInstance()->addSignal( m_sigBigTuxBlink[1], 0, this->getParent(), NULL );
    ...
    if (m_aTux[0] && m_aTux[1] && m_aTux[2]){
      SignalSender::getInstance()->addSignal(m_sigTuxAll, 0, this->getParent(), NULL);
    ...
    if (m_aLinux[0] && m_aLinux[1] && m_aLinux[2] && m_aLinux[3] && m_aLinux[4]){
      SignalSender::getInstance()->addSignal(m_sigLinuxAll, 0, this->getParent(), NULL);
      m_aLinux[0] = m_aLinux[1] = m_aLinux[2] = m_aLinux[3] = m_aLinux[4] = false;
      switch (m_iMultiplier){
      case 1: 
	m_iMultiplier = 2;
	SignalSender::getInstance()->addSignal(m_sigMultiplier[1], 0, this->getParent(), NULL);
	break;
      case 2: 
	m_iMultiplier = 3;
	SignalSender::getInstance()->addSignal(m_sigMultiplier[2], 0, this->getParent(), NULL);
	break;
      case 3: 
	m_iMultiplier = 4;
	SignalSender::getInstance()->addSignal(m_sigMultiplier[3], 0, this->getParent(), NULL);
	break;
      case 4: 
	m_iMultiplier = 5;
	SignalSender::getInstance()->addSignal(m_sigMultiplier[4], 0, this->getParent(), NULL);
	break;
      }
      score->addScore(5000);
    }
