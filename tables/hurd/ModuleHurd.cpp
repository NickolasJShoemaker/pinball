...
public:
  ...
  void onTick(){
	...
	if (m_aArrow[7]){
	  m_aArrow[7] = false;
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[8], 0, this->getParent(), NULL );
  ...
  void onSignal(int em_signal, Group* em_group){
	...
	if (m_iBumpCounter == 10){
	  ...
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[6], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[7], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowBlink[8], 0, this->getParent(), NULL );
	  //cerr << "right ramp arrows blink" << endl;
	  m_iMissionTick = MISSION_TEXT_TIMEOUT;
	}
      }
	...
	SignalSender::getInstance()->addSignal( m_sigBigHurdOn[0], 0, this->getParent(), NULL );
	...
	SignalSender::getInstance()->addSignal( m_sigBigHurdOn[1], 0, this->getParent(), NULL );
	...
	SignalSender::getInstance()->addSignal( m_sigBigHurdOn[2], 0, this->getParent(), NULL );
	...
	SignalSender::getInstance()->addSignal( m_sigBigHurdOn[3], 0, this->getParent(), NULL );
    ...
    // GNU
    else if(em_signal == m_sigGnu[0]){
      if (!m_aGnu[0]){
	SignalSender::getInstance()->addSignal(m_sigGnuOn[0], 0, this->getParent(), NULL);
	m_aGnu[0] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal == m_sigGnu[1]){
      if (!m_aGnu[1]){
	SignalSender::getInstance()->addSignal(m_sigGnuOn[1], 0, this->getParent(), NULL);
	m_aGnu[1] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal == m_sigGnu[2]){
      if (!m_aGnu[2]){
	SignalSender::getInstance()->addSignal(m_sigGnuOn[2], 0, this->getParent(), NULL);
	m_aGnu[2] = true;
      }
      score->addScore(50);
    } 
    // HURD
    else if(em_signal ==  m_sigHurd[0] ){
      if (m_aHurd[0]){
	SignalSender::getInstance()->addSignal(m_sigHurdOff[0], 0, this->getParent(), NULL);
	m_aHurd[0] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigHurdOn[0], 0, this->getParent(), NULL);
	m_aHurd[0] = true;
      }
      score->addScore(500);
    } 
    else if(em_signal ==  m_sigHurd[1] ){
      if (m_aHurd[1]){
	SignalSender::getInstance()->addSignal(m_sigHurdOff[1], 0, this->getParent(), NULL);
	m_aHurd[1] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigHurdOn[1], 0, this->getParent(), NULL);
	m_aHurd[1] = true;
      }
      score->addScore(500);
    } 
    else if(em_signal ==  m_sigHurd[2] ){
      if (m_aHurd[2]){
	SignalSender::getInstance()->addSignal(m_sigHurdOff[2], 0, this->getParent(), NULL);
	m_aHurd[2] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigHurdOn[2], 0, this->getParent(), NULL);
	m_aHurd[2] = true;
      }
      score->addScore(500);
    } 
    else if(em_signal ==  m_sigHurd[3] ){
      if (m_aHurd[3]){
	SignalSender::getInstance()->addSignal(m_sigHurdOff[3], 0, this->getParent(), NULL);
	m_aHurd[3] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigHurdOn[3], 0, this->getParent(), NULL);
	m_aHurd[3] = true;
      }
      score->addScore(500);
    } 
    // MACH
    else if(em_signal ==  m_sigMach[0] ){
      if (m_aMach[0]){
	SignalSender::getInstance()->addSignal(m_sigMachOff[0], 0, this->getParent(), NULL);
	m_aMach[0] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigMachOn[0], 0, this->getParent(), NULL);
	m_aMach[0] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal ==  m_sigMach[1] ){
      if (m_aMach[1]){
	SignalSender::getInstance()->addSignal(m_sigMachOff[1], 0, this->getParent(), NULL);
	m_aMach[1] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigMachOn[1], 0, this->getParent(), NULL);
	m_aMach[1] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal ==  m_sigMach[2] ){
      if (m_aMach[2]){
	SignalSender::getInstance()->addSignal(m_sigMachOff[2], 0, this->getParent(), NULL);
	m_aMach[2] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigMachOn[2], 0, this->getParent(), NULL);
	m_aMach[2] = true;
      }
      score->addScore(50);
    } 
    else if(em_signal ==  m_sigMach[3] ){
      if (m_aMach[3]){
	SignalSender::getInstance()->addSignal(m_sigMachOff[3], 0, this->getParent(), NULL);
	m_aMach[3] = false;
      }else{
	SignalSender::getInstance()->addSignal(m_sigMachOn[3], 0, this->getParent(), NULL);
	m_aMach[3] = true;
      }
      score->addScore(50);
    } 
	...
	if (m_aMach[a] != m_aMach[a+1]){
	  ...
	  if (m_aMach[a]) SignalSender::getInstance()->addSignal(m_sigMachOn[a], 0, this->getParent(), NULL);
	  else SignalSender::getInstance()->addSignal(m_sigMachOff[a], 0, this->getParent(), NULL);
	} 
      } 
      if (m_aMach[3] != tmp){
	m_aMach[3] = tmp;
	if (m_aMach[3]) SignalSender::getInstance()->addSignal(m_sigMachOn[3], 0, this->getParent(), NULL);
	else SignalSender::getInstance()->addSignal(m_sigMachOff[3], 0, this->getParent(), NULL);
      }
      // GNU
      tmp = m_aGnu[0];
	...
	if (m_aGnu[aa] != m_aGnu[aa+1]){
	  ...
	  if (m_aGnu[aa]) SignalSender::getInstance()->addSignal(m_sigGnuOn[aa], 0, this->getParent(), NULL);
	  else SignalSender::getInstance()->addSignal(m_sigGnuOff[aa], 0, this->getParent(), NULL);
	}
      } 
      if (m_aGnu[2] != tmp){
	m_aGnu[2] = tmp;
	if (m_aGnu[2]) SignalSender::getInstance()->addSignal(m_sigGnuOn[2], 0, this->getParent(), NULL);
	else SignalSender::getInstance()->addSignal(m_sigGnuOff[2], 0, this->getParent(), NULL);
      }
    } 
	...
	if (m_aMach[a] != m_aMach[a-1]){
      ...
      if (m_aMach[0] != tmp){
	m_aMach[0] = tmp;
	if (m_aMach[0]) SignalSender::getInstance()->addSignal(m_sigMachOn[0], 0, this->getParent(), NULL);
	else SignalSender::getInstance()->addSignal(m_sigMachOff[0], 0, this->getParent(), NULL);
      }
      // GNU
      tmp = m_aGnu[2];
      { for (int a=2; a>0; --a){
	if (m_aGnu[a] != m_aGnu[a-1]){
	  m_aGnu[a] = m_aGnu[a-1];
	  if (m_aGnu[a]) SignalSender::getInstance()->addSignal(m_sigGnuOn[a], 0, this->getParent(), NULL);
	  else SignalSender::getInstance()->addSignal(m_sigGnuOff[a], 0, this->getParent(), NULL);
	}
      } }
      if (m_aGnu[0] != tmp){
	m_aGnu[0] = tmp;
	if (m_aGnu[0]) SignalSender::getInstance()->addSignal(m_sigGnuOn[0], 0, this->getParent(), NULL);
	else SignalSender::getInstance()->addSignal(m_sigGnuOff[0], 0, this->getParent(), NULL);
      }
    } 
	...
	if (m_aArrow[7]){
	  ...
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[7], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[8], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigBigHurdBlink[3], 0, this->getParent(), NULL );
	...
	SignalSender::getInstance()->addSignal( m_sigArrowOn[6], 0, this->getParent(), NULL );
	SignalSender::getInstance()->addSignal( m_sigArrowOn[7], 0, this->getParent(), NULL );
	SignalSender::getInstance()->addSignal( m_sigArrowOn[8], 0, this->getParent(), NULL );
	...
	if (m_aArrow[4]){
	  ...
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[4], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigArrowOff[5], 0, this->getParent(), NULL );
	  SignalSender::getInstance()->addSignal( m_sigBigHurdBlink[2], 0, this->getParent(), NULL );
    ...
    else if(em_signal == m_sigArrow[2]){
      if (m_iMission == MISSION_ARROWS || m_iMission == MISSION_FIFTH_ALL){
    ...
    if (m_aArrow[0] && m_iMission == MISSION_ARROW){
      ...
      SignalSender::getInstance()->addSignal( m_sigBigHurdBlink[0], 0, this->getParent(), NULL );
      m_aArrow[0] = false;
    ...
    if (m_aArrow[1] && m_aArrow[2] && m_iMission == MISSION_ARROWS){
      ...
      SignalSender::getInstance()->addSignal( m_sigBigHurdBlink[1], 0, this->getParent(), NULL );
    ...
    if (m_aHurd[0] && m_aHurd[1] && m_aHurd[2] && m_aHurd[3]){
      SignalSender::getInstance()->addSignal(m_sigHurdAll, 0, this->getParent(), NULL);
    ...
    if (m_aGnu[0] && m_aGnu[1] && m_aGnu[2]){
      SignalSender::getInstance()->addSignal(m_sigGnuAll, 0, this->getParent(), NULL);
    ...
    if (m_aMach[0] && m_aMach[1] && m_aMach[2] && m_aMach[3]){
      SignalSender::getInstance()->addSignal(m_sigMachAll, 0, this->getParent(), NULL);
