...
public:
    ...
    if (table){
        if (m_bBallColourChanged == false){
            ...
            for (int i = 0; i < m_iMultiplier + 1; i++)
              SignalSender::getInstance()->addSignal( m_sig0123On[i], 0, this->getParent(), NULL );
            for (int i = 0; i < m_iGplVersionLevel + 1; i++)
              SignalSender::getInstance()->addSignal( m_sigv123On[i], 0, this->getParent(), NULL );
    ...
    if (m_aLocked[0] == false && m_aLocked[1] == false){
      ...
      if (m_iLeftLockArrowTick >= 20){
        if (m_aLockArrow[0][0]){
          ...
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[1][0], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[0][0], 0, this->getParent(), NULL );
        } else if (m_aLockArrow[1][0]){
          m_aLockArrow[1][0] = false;
          m_aLockArrow[2][0] = true;
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[2][0], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[1][0], 0, this->getParent(), NULL );
        } else if (m_aLockArrow[2][0]){
          m_aLockArrow[2][0] = false;
          m_aLockArrow[3][0] = true;
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[3][0], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[2][0], 0, this->getParent(), NULL );
        } else if (m_aLockArrow[3][0]){
          m_aLockArrow[3][0] = false;
          m_aLockArrow[4][0] = true;
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[4][0], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[3][0], 0, this->getParent(), NULL );
        } else if (m_aLockArrow[4][0]){
          m_aLockArrow[4][0] = false;
          m_aLockArrow[5][0] = true;
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[5][0], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[4][0], 0, this->getParent(), NULL );
        } else if (m_aLockArrow[5][0]){
          m_aLockArrow[5][0] = false;
          m_aLockArrow[0][0] = true;
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[0][0], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[5][0], 0, this->getParent(), NULL );
        }
    ...
    if (m_aLocked[0] == true && m_aLocked[1] == false){
      ...
      if (m_iRightLockArrowTick >= 20){
        if (m_aLockArrow[0][1]){
          ...
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[1][1], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[0][1], 0, this->getParent(), NULL );
        } else if (m_aLockArrow[1][1]){
          m_aLockArrow[1][1] = false;
          m_aLockArrow[2][1] = true;
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[2][1], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[1][1], 0, this->getParent(), NULL );
        } else if (m_aLockArrow[2][1]){
          m_aLockArrow[2][1] = false;
          m_aLockArrow[3][1] = true;
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[3][1], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[2][1], 0, this->getParent(), NULL );
        } else if (m_aLockArrow[3][1]){
          m_aLockArrow[3][1] = false;
          m_aLockArrow[4][1] = true;
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[4][1], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[3][1], 0, this->getParent(), NULL );
        } else if (m_aLockArrow[4][1]){
          m_aLockArrow[4][1] = false;
          m_aLockArrow[5][1] = true;
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[5][1], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[4][1], 0, this->getParent(), NULL );
        } else if (m_aLockArrow[5][1]){
          m_aLockArrow[5][1] = false;
          m_aLockArrow[0][1] = true;
          SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[0][1], 0, this->getParent(), NULL );
          SignalSender::getInstance()->addSignal( m_sigLockArrowOff[5][1], 0, this->getParent(), NULL );
        }
  ...
  void onSignal(int em_signal, Group* em_group){
    ...
    else if(em_signal == m_sigFree[0]){
      if (!m_aFree[0]){
	SignalSender::getInstance()->addSignal(m_sigFreeOn[0], 0, this->getParent(), NULL);
	m_aFree[0] = true;
      }
      score->addScore(withMultiplier(50));
    } 
    else if(em_signal == m_sigFree[1]){
      if (!m_aFree[1]){
	SignalSender::getInstance()->addSignal(m_sigFreeOn[1], 0, this->getParent(), NULL);
	m_aFree[1] = true;
      }
      score->addScore(withMultiplier(50));
    } 
    else if(em_signal == m_sigFree[2]){
      if (!m_aFree[2]){
	SignalSender::getInstance()->addSignal(m_sigFreeOn[2], 0, this->getParent(), NULL);
	m_aFree[2] = true;
      }
      score->addScore(withMultiplier(50));
    } 
    else if(em_signal == m_sigFree[3]){
      if (!m_aFree[3]){
	SignalSender::getInstance()->addSignal(m_sigFreeOn[3], 0, this->getParent(), NULL);
	m_aFree[3] = true;
      }
      score->addScore(withMultiplier(50));
    } 
    else if(em_signal == m_sigKnockdown[0][0]){
      if (!m_aKnockdown[0]){
	SignalSender::getInstance()->addSignal(m_sigCopyleftOn[0], 0, this->getParent(), NULL);
	m_aKnockdown[0] = true;
        score->addScore(withMultiplier(50));
      }
    } 
    else if(em_signal == m_sigKnockdown[1][0]){
      if (!m_aKnockdown[1]){
	SignalSender::getInstance()->addSignal(m_sigCopyleftOn[1], 0, this->getParent(), NULL);
	m_aKnockdown[1] = true;
        score->addScore(withMultiplier(50));
      }
    } 
    else if(em_signal == m_sigKnockdown[2][0]){
      if (!m_aKnockdown[2]){
	SignalSender::getInstance()->addSignal(m_sigCopyleftOn[2], 0, this->getParent(), NULL);
	m_aKnockdown[2] = true;
        score->addScore(withMultiplier(50));
      }
    } 
    else if(em_signal == m_sigKnockdown[3][0]){
      if (!m_aKnockdown[3]){
	SignalSender::getInstance()->addSignal(m_sigCopyleftOn[3], 0, this->getParent(), NULL);
	m_aKnockdown[3] = true;
        score->addScore(withMultiplier(50));
      }
    } 
    else if(em_signal == m_sigKnockdown[4][0]){
      if (!m_aKnockdown[4]){
	SignalSender::getInstance()->addSignal(m_sigCopyleftOn[4], 0, this->getParent(), NULL);
	m_aKnockdown[4] = true;
        score->addScore(withMultiplier(50));
      }
    } 
    else if(em_signal == m_sigKnockdown[5][0]){
      if (!m_aKnockdown[5]){
	SignalSender::getInstance()->addSignal(m_sigCopyleftOn[5], 0, this->getParent(), NULL);
	m_aKnockdown[5] = true;
        score->addScore(withMultiplier(50));
      }
    } 
    else if(em_signal == m_sigKnockdown[6][0]){
      if (!m_aKnockdown[6]){
	SignalSender::getInstance()->addSignal(m_sigCopyleftOn[6], 0, this->getParent(), NULL);
	m_aKnockdown[6] = true;
        score->addScore(withMultiplier(50));
      }
    }
    else if(em_signal == m_sigKnockdown[7][0]){
      if (!m_aKnockdown[7]){
	SignalSender::getInstance()->addSignal(m_sigCopyleftOn[7], 0, this->getParent(), NULL);
	m_aKnockdown[7] = true;
        score->addScore(withMultiplier(50));
      }
    }
    else if(em_signal == m_sigKnockdown[8][0]){
      if (!m_aKnockdown[8]){
	SignalSender::getInstance()->addSignal(m_sigGplOn[0], 0, this->getParent(), NULL);
	m_aKnockdown[8] = true;
        score->addScore(withMultiplier(50));
      }
    }
    else if(em_signal == m_sigKnockdown[9][0]){
      if (!m_aKnockdown[9]){
	SignalSender::getInstance()->addSignal(m_sigGplOn[1], 0, this->getParent(), NULL);
	m_aKnockdown[9] = true;
        score->addScore(withMultiplier(50));
      }
    }
    else if(em_signal == m_sigKnockdown[10][0]){
      if (!m_aKnockdown[10]){
	SignalSender::getInstance()->addSignal(m_sigGplOn[2], 0, this->getParent(), NULL);
	m_aKnockdown[10] = true;
        score->addScore(withMultiplier(50));
      }
    }
    else if(em_signal == m_sigMidStopperBreached){
      for (int i = 0; i < 6; i++){
          SignalSender::getInstance()->addSignal( m_sigArrowOff[i], 0, this->getParent(), NULL );
          m_aArrow[i] = false;
        }
    ...
    else if(em_signal == m_sigBottomStopperBreached){
    ...
    else if(em_signal == m_sigSideStopperBreached){
      ...
      for (int i = 0; i < 6; i++)
        SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[i][0], 0, this->getParent(), NULL );
      ...
      for (int i = 0; i < 6; i++)
        SignalSender::getInstance()->addSignal( m_sigLockArrowBlink[i][1], 0, this->getParent(), NULL );
    }
    else if(em_signal ==  PBL_SIG_BALL_ON ){
    Table * table = Table::getInstance();
    ...
    else if(em_signal ==  m_sigFreeballTarget){
    ...
    else if(em_signal ==  m_sigFreedomTarget){
      if (m_iMultiplier == 3){
    ...
    if (m_aFree[0] && m_aFree[1] && m_aFree[2] && m_aFree[3]){
      SignalSender::getInstance()->addSignal(m_sigFreeAll, 0, this->getParent(), NULL);
      ...
      if (m_aLocked[0] == true && m_aLocked[1] == true){
        ...
        unLockBalls();
        SignalSender::getInstance()->addSignal(m_sigLockRelease, 0, this->getParent(), NULL);
        m_bMultiBall = true; 
        ...
        for (int j = 0; j < 2; j++)
          for (int i = 0; i < 6; i++)
            SignalSender::getInstance()->addSignal( m_sigLockArrowCrazyBlink[i][j], 0, this->getParent(), NULL );
        for (int j = 0; j < 2; j++)
          for (int i = 0; i < 6; i++){
            SignalSender::getInstance()->addSignal( m_sigLockArrowOn[i][j], 600, this->getParent(), NULL );
            m_aLockArrow[i][j] = false;
          }
        for (int i = 0; i < 6; i++)
    ...
    if (m_aKnockdown[0] && m_aKnockdown[1] && m_aKnockdown[2] && 
      ...
      int i;
      SignalSender::getInstance()->addSignal(m_sigCopyleftAll, 0, this->getParent(), NULL);
      score->addScore(withMultiplier(500));
      for (i = 0; i < 8; i++){
          if (m_aKnockdown[i]){
              SignalSender::getInstance()->addSignal(m_sigKnocking[i][1], 250, this->getParent(), NULL);
              m_aKnockdown[i] = false;
            }
        }
	SignalSender::getInstance()->addSignal(m_sigCopyleftKnockdownReset, 300, this->getParent(), NULL);
        m_iMultiplier++;
        if (m_iMultiplier > 3)
            m_iMultiplier = 3;
        else
          {
          SignalSender::getInstance()->addSignal(m_sig0123On[m_iMultiplier], 50, this->getParent(), NULL);
          SignalSender::getInstance()->addSignal(m_sig0123Announce[m_iMultiplier], 0, this->getParent(), NULL);
          }
    ...
    if (m_aKnockdown[8] && m_aKnockdown[9] && m_aKnockdown[10]){
      SignalSender::getInstance()->addSignal(m_sigGplAll, 0, this->getParent(), NULL);
      score->addScore(withMultiplier(500));
      for (int i = 8; i < 11; i++){
          if (m_aKnockdown[i]){
              SignalSender::getInstance()->addSignal(m_sigKnocking[i][1], 250, this->getParent(), NULL);
	...
	SignalSender::getInstance()->addSignal(m_sigGplKnockdownReset, 300, this->getParent(), NULL);
        if (m_iGplVersionLevel < 2){
          m_iGplVersionLevel++;
          for (int i = 0; i <= m_iGplVersionLevel; i++)
            SignalSender::getInstance()->addSignal(m_sigv123On[i], 50, this->getParent(), NULL);
        ...
        if (m_iGplVersionLevel == 2 && m_bFreeBallTargetOn == false && 
          ...
          SendSignal (m_sigv123Announce[m_iGplVersionLevel], 0, this->getParent(), NULL);
          for (int i = 1; i < 4; i++)
            SignalSender::getInstance()->addSignal(m_sigv123Off[i], 60, this->getParent(), NULL);
          SignalSender::getInstance()->addSignal(m_sigv123On[0], 60, this->getParent(), NULL);
        }
