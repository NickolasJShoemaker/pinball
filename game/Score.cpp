...
void Score::onSignal(int em_signal, Group* em_group){
  EM_COUT((int)em_signal, 1);

  if(em_signal ==  PBL_SIG_RESET_ALL ){
    this->clear();
    this->setText1("press enter to launch ball");
    SignalSender::getInstance()->addSignal( PBL_SIG_CLEAR_TEXT, 400, this->getParent(), NULL );
  } 
  else if(em_signal ==  PBL_SIG_TILT ){
    this->clearText();
    this->setText1("tilt");
    SignalSender::getInstance()->addSignal( PBL_SIG_RESET_ALL, 600, this->getParent(), NULL );
  }
  else if(em_signal ==  PBL_SIG_TILT_WARNING ){
    this->clearText();
    this->setText1("warning");
    SignalSender::getInstance()->addSignal( PBL_SIG_CLEAR_TEXT, 400, this->getParent(), NULL );
  }
  else if(em_signal ==  PBL_SIG_CLEAR_TEXT ){
    this->clearText();
  }
  ...
  else if(em_signal ==  Loader::getInstance()->getSignal("extraball") ){
    this->m_bExtraBall = true;
  }
  else if(em_signal ==  PBL_SIG_GAME_OVER ){
    this->clearText();
