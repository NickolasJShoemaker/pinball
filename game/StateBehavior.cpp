...
void StateBehavior::setState(StateItem* stateitem){
  ...
  // apply light
  if(this->getLight() != NULL) this->getLight()->setOn(p_CurrentStateItem->m_bLight);
  // zero counter
  p_CurrentStateItem->m_iTick = 0;
}

void StateBehavior::onSignal(int em_signal, Group* em_group){
  EmAssert(this->getParent() != NULL, "Parent NULL");
...
#if EM_DEBUG
	if (Loader::getInstance()->getSignal(em_signal) != NULL){
		EM_COUT("Got signal " << Loader::getInstance()->getSignal(em_signal), 0);
	}
#endif
...
void StateBehavior::onCollision(const Vertex3D& em_vtx, const Vertex3D& em_vtxOwn, Group* em_group){
  if (p_CurrentStateItem == NULL && m_vStateItem.size() == 0) return;
  ...
  if(em_group != NULL && (em_group->getUserProperties() & PBL_BALL)){
    ...
    if (p_CurrentStateItem->m_iCollSig >= 0){
      SignalSender::getInstance()->addSignal(p_CurrentStateItem->m_iCollSig, p_CurrentStateItem->m_iSigDelay, 
		 this->getParent(), NULL);
    }
    if (p_CurrentStateItem->m_iCollState >= 0){
      this->setState(this->getStateItem(p_CurrentStateItem->m_iCollState));
    }
    EM_COUT("StateBehavior::onCollision(const Vertex3D& em_vtx, const Vertex3D& em_vtxOwn, Group* em_group) ball", 0);
  }
}
