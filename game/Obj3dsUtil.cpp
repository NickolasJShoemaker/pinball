...
#include <stdio.h>
  ...
  char* skipafter(char* src, const char* pat);
...
#define BOUND 1000.
  ...
  const char* keyword[] = { "Tri-mesh,", "Vertex list", "Face list",
...
#define SMOOTHING 6
  ...
  int fprintObject(FILE* f, Object const* const o){
    ...
    for(i=0;i<o->nv;i++){
      fprintf( f,"Vertex %d: X: %f Y: %f Z: %f ",i , o->vv[i].x,  o->vv[i].y, o->vv[i].z);
      fprintf( f,"\t U: %f V: %f \r\n", o->vv[i].u,  o->vv[i].v);
    }
    fprintf(f,"\r\nFace list\r\n");
    for(i=0;i<o->nf;i++){
      fprintf( f,"Face %d: A:%d B:%d C:%d",i, o->vf[i].a,  o->vf[i].b, o->vf[i].c);
...
#define alpha(x) ( (x>>24) & 0x0000FF )
  ...
  int convertColorRGBFloatToAnsi(float const r, float const g, float const b, int* res){
    //debugf("TODO");
    *res=r+g+b;
    exit(0);
  }
  ...
  int convertColorStringRGBAToRGBA8(char const* const p, unsigned char *r, unsigned char* g, unsigned char* b,unsigned  char* a, unsigned int* dest){
  ...
  int convertColorStringRGBAToRGBAFloat(char const * const p, float *r, float* g, float* b, float* a, unsigned int* dest){
  ...
  int convertColorStringToRGBAFloat(char const * const p, float *r, float* g, float* b, float* a, unsigned int* dest){
