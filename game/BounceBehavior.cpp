...
void BounceBehavior::onTick(){
    ...
    EmAssert(this->getParent()->getUserProperties() & PBL_BALL, "BounceBehavior::onTick()");
    //((BallGroup*)this->getParent())->tick();
    BallGroup* temp = (BallGroup*)this->getParent();
    temp->tick();
}
void BounceBehavior::onSignal(int em_signal, Group* em_group){
    ...
    if(em_signal ==  PBL_SIG_BNUDGE ){
        m_vtxDir.z -= SPEED_FCT*0.1f;
    } 
    else if(em_signal ==  PBL_SIG_TNUDGE ){
        m_vtxDir.z += SPEED_FCT*0.1f;
    }
    else if(em_signal ==  PBL_SIG_LNUDGE ){
        m_vtxDir.x -= SPEED_FCT*0.1f;
    } 
    else if(em_signal ==  PBL_SIG_RNUDGE ){
        m_vtxDir.x += SPEED_FCT*0.1f;
    }
    else if(em_signal ==  Loader::getInstance()->getSignal("stop_balls") ){
        m_bStopped = true;
    }
    else if(em_signal ==  Loader::getInstance()->getSignal("resume_balls") ){
        m_bStopped = false;
    }
}
...
void BounceBehavior::onCollision(const Vertex3D& vtxW, const Vertex3D& vtxOwn, Group* pGroup){
    ...
    if (iindex < ARM_TABLE_SIZE-2){
      ...
      vtxDir.x = EMath::quadratic(g_vtxArmTable[iindex].x, g_vtxArmTable[iindex+1].x, g_vtxArmTable[iindex+2].x, foffset);
      vtxDir.z = EMath::quadratic(g_vtxArmTable[iindex].z, g_vtxArmTable[iindex+1].z, g_vtxArmTable[iindex+2].z, foffset);
    } else if (iindex == ARM_TABLE_SIZE-2){
      vtxDir.x = EMath::quadratic(g_vtxArmTable[iindex].x, g_vtxArmTable[iindex+1].x, 0.0f, foffset);
      vtxDir.z = EMath::quadratic(g_vtxArmTable[iindex].z, g_vtxArmTable[iindex+1].z, 0.0f, foffset);
    }else{
