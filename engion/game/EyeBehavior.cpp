...
void EyeBehavior::onSignal(int em_signal, Group* em_group){
...
void EyeBehavior::onTick(){
  ...
  if (m_iNudgeTick < 1){
    ...
    if (Keyboard::isKeyDown(Config::getInstance()->getKey(bottomnudge)) && !m_bTilt){
      ...
      SignalSender::getInstance()->addSignal(PBL_SIG_BNUDGE, 0, this->getParent(), NULL ); 
      ...
      SignalSender::getInstance()->addSignal(PBL_SIG_LNUDGE, 0, this->getParent(), NULL ); 
      SoundUtil::getInstance()->playSample(m_iSound, false);
      ...
      SignalSender::getInstance()->addSignal(PBL_SIG_RNUDGE, 0, this->getParent(), NULL ); 
      SoundUtil::getInstance()->playSample(m_iSound, false);
    }
    ...
    switch (m_iNudgeType){
    case PBL_SIG_BNUDGE: 
      SignalSender::getInstance()->addSignal(PBL_SIG_TNUDGE, 0, this->getParent(), NULL ); 
      break;
    case PBL_SIG_LNUDGE: 
      SignalSender::getInstance()->addSignal(PBL_SIG_RNUDGE, 0, this->getParent(), NULL ); 
      break;
  ...
  if (m_iTiltTick > 200){
    // send tilt signal
    m_bTilt = true;
    SignalSender::getInstance()->addSignal( PBL_SIG_TILT, 0, this->getParent(), NULL ); 
    EM_COUT("EyeBehavior::onTick() TILT", 1);
    m_iTiltTick = 0;
  } else if (m_iTiltTick > 150){
    // send warning signal
    SignalSender::getInstance()->addSignal( PBL_SIG_TILT_WARNING, 0, this->getParent(), NULL ); 
