...
#ifndef TEXANIMATION_HPP
#define TEXANIMATION_HPP
...
public:
	...
	TexAnimation(int step, int nvtx): Behavior(){
		EmAssert(nvtx > 0, "TexAnimation::TexAnimation nvtx less than 1");
		m_iStep = step;
		m_iTick = 0;
		m_iTexCoord = nvtx;
	}
	~TexAnimation() = default;
	...
	void onTick(){
		...
		for (int a=0; a<m_iTexCoord && a<4; a++){
			...
			*/
			...
			if (b != NULL){
				b->setUV(u, v, a);
				EM_COUT("TexAnimation::onTick() " << u <<" "<< v <<" size "<< size <<" index "<< index <<" tick "<< m_iTick, 0);
			}
		}
