...
constexpr float EM_PI   		= 3.1415926f;
constexpr float EM_PI_DIV_2 = 1.5707963f;
constexpr float EM_PI_2	    = 6.2831853f;
constexpr float EM_SQRT_2		= 1.4142135f;
constexpr float EM_SIN_45		= 0.7071069f;
constexpr float EM_SIN_60   = 0.8660254f;
constexpr float EM_SQRT_3   =	1.73205f;
...
class EMath {
 public:
  ...
  inline static void applyMatrix(const Matrix & mtx, const Vertex3D & vtxIn, Vertex3D & vtxOut){
    vtxOut.x = vtxIn.x * mtx.v[0][0] + vtxIn.y * mtx.v[0][1] + vtxIn.z * mtx.v[0][2] + mtx.t[0];
    vtxOut.y = vtxIn.x * mtx.v[1][0] + vtxIn.y * mtx.v[1][1] + vtxIn.z * mtx.v[1][2] + mtx.t[1];
    vtxOut.z = vtxIn.x * mtx.v[2][0] + vtxIn.y * mtx.v[2][1] + vtxIn.z * mtx.v[2][2] + mtx.t[2];
  };
  inline static void applyMatrixRot(const Matrix & mtx, const Vertex3D & vtxIn, Vertex3D & vtxOut){
    vtxOut.x = vtxIn.x * mtx.v[0][0] + vtxIn.y * mtx.v[0][1] + vtxIn.z * mtx.v[0][2];
    vtxOut.y = vtxIn.x * mtx.v[1][0] + vtxIn.y * mtx.v[1][1] + vtxIn.z * mtx.v[1][2];
    vtxOut.z = vtxIn.x * mtx.v[2][0] + vtxIn.y * mtx.v[2][1] + vtxIn.z * mtx.v[2][2];
  };
  ...
  static float polygonZNormal(const Vertex3D & edgeA, const Vertex3D & edgeB, const Vertex3D & edgeC);

  void planeLineIntersection(const Vertex3D & nrml, float dist, const Vertex3D & vtxA, const Vertex3D & vtxB, Vertex3D & vtxDiff);
  ...
  static void reflectionDamp(const Vertex3D & vtxIn, const Vertex3D & vtxWall, Vertex3D & vtxOut, float damp, float extra, 
